...
import pm.diagram.part.Messages;
import pm.diagram.part.PmVisualIDRegistry;

/**
 * @generated
 */
public class PmNavigatorContentProvider implements ICommonContentProvider {
	...
	public PmNavigatorContentProvider() {
	...
	public Object[] getChildren(Object parentElement) {
		if (parentElement instanceof IFile) {
			...
			Resource resource = myEditingDomain.getResourceSet().getResource(
					fileURI, true);
			ArrayList<PmNavigatorItem> result = new ArrayList<PmNavigatorItem>();
			...
			result.addAll(createNavigatorItems(
					selectViewsByType(topViews, DocumentRootEditPart.MODEL_ID),
					file, false));
			return result.toArray();
		}

		if (parentElement instanceof PmNavigatorGroup) {
			PmNavigatorGroup group = (PmNavigatorGroup) parentElement;
			return group.getChildren();
		}

		if (parentElement instanceof PmNavigatorItem) {
			PmNavigatorItem navigatorItem = (PmNavigatorItem) parentElement;
	...
	private Object[] getViewChildren(View view, Object parentElement) {
		...
		case ProjectParticipantsEditPart.VISUAL_ID: {
			...
			Edge sv = (Edge) view;
			PmNavigatorGroup target = new PmNavigatorGroup(
					Messages.NavigatorGroupName_ProjectParticipants_4018_target,
					"icons/linkTargetNavigatorGroup.gif", parentElement); //$NON-NLS-1$
			PmNavigatorGroup source = new PmNavigatorGroup(
					Messages.NavigatorGroupName_ProjectParticipants_4018_source,
					"icons/linkSourceNavigatorGroup.gif", parentElement); //$NON-NLS-1$
			Collection<View> connectedViews;
			connectedViews = getLinksTargetByType(Collections.singleton(sv),
					PmVisualIDRegistry.getType(EmployeeEditPart.VISUAL_ID));
		...
		case ProjectEditPart.VISUAL_ID: {
			...
			Node sv = (Node) view;
			PmNavigatorGroup outgoinglinks = new PmNavigatorGroup(
					Messages.NavigatorGroupName_Project_2005_outgoinglinks,
					"icons/outgoingLinksNavigatorGroup.gif", parentElement); //$NON-NLS-1$
			PmNavigatorGroup incominglinks = new PmNavigatorGroup(
					Messages.NavigatorGroupName_Project_2005_incominglinks,
					"icons/incomingLinksNavigatorGroup.gif", parentElement); //$NON-NLS-1$
			Collection<View> connectedViews;
			connectedViews = getOutgoingLinksByType(Collections.singleton(sv),
					PmVisualIDRegistry
	 ...
	 * @generated
	 */
	private Collection<PmNavigatorItem> createNavigatorItems(
			Collection<View> views, Object parent, boolean isLeafs) {
		ArrayList<PmNavigatorItem> result = new ArrayList<PmNavigatorItem>(
				views.size());
		for (View nextView : views) {
			result.add(new PmNavigatorItem(nextView, parent, isLeafs));
		}
		return result;
	}
	...
	public Object getParent(Object element) {
		if (element instanceof PmAbstractNavigatorItem) {
			PmAbstractNavigatorItem abstractNavigatorItem = (PmAbstractNavigatorItem) element;
